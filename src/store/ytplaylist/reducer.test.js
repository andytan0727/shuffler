...
describe("ytplaylist reducer", () => {
  ...
  const randomIds = Array.from({ length: 2 }, (_) =>
  ...
  test("should handle REMOVE_FROM_LIST_TO_PLAY", () => {
    ...
    expect(
      ytplaylistReducer(
        ...
        // @ts-ignore
        removeFromListToPlayAction([playlist.id], "playlist")
      )
    ).toEqual({
      ...stateWithExistingLTP,
      listToPlay: [...video.items],
    ...
    expect(
      ytplaylistReducer(
        ...
        // @ts-ignore
        removeFromListToPlayAction([video.id], "video")
      )
    ).toEqual({
      ...stateWithExistingLTP,
      listToPlay: [...playlist.items],
    ...
    expect(
      ytplaylistReducer(
        ...
        // @ts-ignore
        removeFromListToPlayAction([], "video")
      )
    ...
    expect(
      ytplaylistReducer(
        ...
        // @ts-ignore
        removeFromListToPlayAction([], "playlist")
      )
    ).toEqual(stateWithExistingLTP);
  });
  ...
  test("should handle UPDATE_LIST_TO_PLAY", () => {
    expect(
      ytplaylistReducer(
        ...
        // @ts-ignore
        updateListToPlay(playlist.items)
      )
    ).toEqual({
      ...initialState,
      listToPlay: playlist.items,
